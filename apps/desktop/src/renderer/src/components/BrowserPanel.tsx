...
export default function BrowserPanel(): JSX.Element {
  ...
  const newestUpdateSummary = update?.message ?? 'Update status is ready to check.';
  return <div className="browser-panel">
    ...
    <div ref={surface} className="browser-surface">
      {state.showingHome && <section className="browser-home"><div className="browser-home-content"><ForgeBrand compact /><p className="eyebrow">PRIVATE WORKSPACE BROWSER</p><h2>Research in your own atmosphere.</h2><p>Open public research alongside your project without giving websites access to your files, shell, or credentials.</p><form onSubmit={(event) => { event.preventDefault(); void navigate(); }}><input value={address} onChange={(event) => setAddress(event.target.value)} placeholder="Search or enter a public website" aria-label="Home browser address" /><button className="accent" type="submit">Open site</button></form><div className="browser-home-grid"><article><strong>Newest updates</strong><span>{newestUpdateTitle}</span><small>{newestUpdateSummary}</small><button onClick={() => void checkForUpdates()}>Check for updates</button></article><article><strong>Project</strong><span>Follow source, releases, issues, and release notes on GitHub.</span><button onClick={() => void navigate('https://github.com/kaeganscott26/FORGE')}>Open FORGE on GitHub</button></article></div></div></section>}
