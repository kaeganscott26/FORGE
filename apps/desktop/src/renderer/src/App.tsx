...
export default function App(): JSX.Element {
  ...
  return <main className="app-shell">
    <header className="app-header"><div className="brand"><span>F</span> FORGE <small>v{updateStatus?.currentVersion ?? '2.1.0-beta.2-dev'} · {workspace?.name ?? 'No workspace'}</small></div><div className="toolbar">{updateStatus?.state === 'downloaded' ? <button className="update-ready" onClick={installUpdate}>Restart to update</button> : <button onClick={checkForUpdates} disabled={checkingUpdate || ['checking', 'available', 'downloading'].includes(updateStatus?.state ?? '')}>{updateStatus?.state === 'available' ? `Preparing v${updateStatus.availableVersion}…` : updateStatus?.state === 'downloading' ? 'Downloading update…' : checkingUpdate || updateStatus?.state === 'checking' ? 'Checking…' : 'Check for updates'}</button>}<button className={browserOpen ? 'active-tab' : ''} disabled={!workspace} onClick={() => setBrowserOpen((open) => !open)}>Browser</button><button onClick={openReleases}>Releases</button><button onClick={() => setSettingsOpen('github')}>GitHub</button><button onClick={() => setSettingsOpen('api')}>Settings</button><button title="Open workspace (⌘/Ctrl+O)" onClick={openWorkspace}>Open workspace</button><button disabled={!workspace} onClick={() => void createEntry('file')}>New file</button><button title="Save (⌘/Ctrl+S)" disabled={!active || content === savedContent} onClick={save}>Save</button><button disabled={!selectedNode} className="danger" onClick={deleteActive}>Delete</button></div></header>
