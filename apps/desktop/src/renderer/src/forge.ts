...
async function fallbackResponse<C extends IPCChannel>(channel: C): Promise<IPCResult<any>> {
  ...
  switch (channel) {
    ...
    case 'app.build.info.copy':
      return { success: true, data: { version: '1.1.0-alpha.3-dev', channel: 'development', commit: 'unavailable in browser preview', buildDate: new Date().toISOString(), runtime: 'development', rendererSource: 'development URL', platform: navigator.platform, architecture: 'browser' } } as any;
    case 'settings.get':
