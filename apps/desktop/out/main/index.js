...
const TEXT_EXTENSIONS = /* @__PURE__ */ new Set(["md", "txt", "ts", "tsx", "js", "jsx", "json", "css", "html", "py", "cpp", "c", "h", "java", "rs", "go", "yml", "yaml"]);
...
class WorkspaceService extends EventEmitter {
  ...
  async open(rootPath) {
    ...
    return { rootPath: this.rootPath, name: path.basename(this.rootPath), gitRoot, createdAt: stat.birthtimeMs };
  }
  info() {
    return this.rootPath ? { rootPath: this.rootPath, name: path.basename(this.rootPath), gitRoot: null, createdAt: 0 } : null;
  }
  async close() {
    this.watcher?.close();
    this.watcher = void 0;
  ...
  async list(relativePath = "") {
    return this.listDirectory(await this.resolve(relativePath), relativePath, 0);
  }
  async readFile(relativePath) {
    const absolute = await this.resolve(relativePath);
    const stat = await promises.stat(absolute);
    if (!stat.isFile()) throw new Error("Path is not a file.");
    if (!TEXT_EXTENSIONS.has(path.extname(absolute).slice(1).toLowerCase())) throw new Error("Forge only opens supported text files.");
    return { path: relativePath, content: await promises.readFile(absolute, "utf8"), modifiedAt: stat.mtimeMs };
  }
  ...
  async create(relativePath, type, content = "") {
    const absolute = await this.resolve(relativePath, true);
    if (type === "directory") await promises.mkdir(absolute, { recursive: false });
    else {
      await promises.mkdir(path.dirname(absolute), { recursive: true });
      await promises.writeFile(absolute, content, { flag: "wx" });
    }
  ...
  async parse(relativePath) {
    return parseMarkdown((await this.readFile(relativePath)).content);
  }
  watch() {
  ...
  async listDirectory(absolute, relative, depth) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && depth < 1) node.children = await this.listDirectory(childAbsolute, childRelative, depth + 1);
      nodes.push(node);
    }
  ...
  async resolve(input, allowMissing = false) {
    ...
    const inspect = allowMissing ? path.dirname(candidate) : candidate;
...
class StorageService {
  ...
  async dashboard() {
    ...
    return { id: String(project.id), name: String(project.name), rootPath: String(project.root_path), createdAt: Number(project.created_at), updatedAt: Number(project.updated_at), goals: this.goals(String(project.id)), tasks: this.tasks(String(project.id)) };
  }
  async createGoal(title, description) {
    ...
    this.ready().run("INSERT INTO goals VALUES (?, ?, ?, ?, ?, ?, ?)", [goal.id, project.id, goal.title, description ?? null, goal.status, now, now]);
    await this.persist();
    return goal;
  }
  async createTask(title, description, priority = "medium") {
    ...
    if (!project) throw new Error("No project metadata exists.");
    const now = Date.now();
    const task = { id: id(), title: title.trim(), description, status: "todo", priority, createdAt: now, updatedAt: now };
    this.ready().run("INSERT INTO tasks VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [task.id, project.id, task.title, description ?? null, task.status, priority, now, now]);
    await this.persist();
    return task;
  }
  async ensureProject() {
  ...
  goals(projectId) {
    return this.all("SELECT * FROM goals WHERE project_id = ? ORDER BY created_at DESC", [projectId]).map((row) => ({ id: String(row.id), title: String(row.title), description: row.description ? String(row.description) : void 0, status: String(row.status), createdAt: Number(row.created_at), updatedAt: Number(row.updated_at) }));
  }
  tasks(projectId) {
    return this.all("SELECT * FROM tasks WHERE project_id = ? ORDER BY created_at DESC", [projectId]).map((row) => ({ id: String(row.id), title: String(row.title), description: row.description ? String(row.description) : void 0, status: String(row.status), priority: String(row.priority), createdAt: Number(row.created_at), updatedAt: Number(row.updated_at) }));
  }
  all(sql, params = []) {
  ...
  async persist() {
    if (this.db && this.filePath) await promises.writeFile(this.filePath, this.db.export());
  }
}
...
function registerHandlers() {
  register(IPC_CHANNELS.workspaceOpen, async () => {
    ...
    await storage.close();
    const info = await workspace.open(selection.filePaths[0]);
...
function createWindow() {
  const window = new BrowserWindow({ width: 1500, height: 950, minWidth: 1100, minHeight: 700, show: false, title: "Forge", webPreferences: { preload: join(__dirname, "../preload/index.js"), contextIsolation: true, nodeIntegration: false, sandbox: false } });
...
function appBuildInfo() {
  ...
  return {
    ...identity,
    commit: "a91a4b07b8727eb0416588d7c968748a8765daca",
    buildDate: "2026-08-07T19:15:02.562Z",
