...
- Tags use `v<package-version>`; the current beta target is `v2.1.0-beta.1` and the public release name is `FORGE beta 2.1`.
...
Use normal SemVer syntax. For this release every workspace manifest and `package-lock.json` must report `2.1.0-beta.1`. The public tag is `v2.1.0-beta.1`, and diagnostics report the Beta channel.
...
git cat-file -t v2.1.0-beta.1
git rev-parse v2.1.0-beta.1^{}
```
...
Pushing the annotated `v2.1.0-beta.1` tag triggers `.github/workflows/package-mac.yml`. It checks out the tag, installs Node 22 dependencies, runs typecheck/lint/tests, creates or reconciles a draft, packages a universal app, uploads assets serially, and publishes only after upload verification.
...
For `v2.1.0-beta.1`, upload the universal DMG, universal ZIP, both blockmaps, then `beta-mac.yml`. Uploads are serial. Metadata is last so no updater can discover an incomplete payload set.
...
shasum -a 256 dist_electron/FORGE-2.1.0-beta.1-*.dmg
shasum -a 256 dist_electron/FORGE-2.1.0-beta.1-*.zip
shasum -a 256 dist_electron/FORGE-2.1.0-beta.1-*.blockmap
...
Diagnostics must report `FORGE v2.1.0-beta.1`, `Channel: beta`, the exact source commit, packaged runtime, `file:// packaged app.asar`, platform, architecture, and build date.
...
- [ ] GitHub exposes `FORGE beta 2.1` at `v2.1.0-beta.1`; obsolete local output and caches are cleaned.
