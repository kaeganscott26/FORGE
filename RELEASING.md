...
- Tags use `v<package-version>`; the current beta target is `v2.1.0-beta.2` and the public release name is `FORGE beta 2.1`.
...
For `v2.1.0-beta.2`, upload the universal DMG, universal ZIP, both blockmaps, then `beta-mac.yml`. Uploads are serial. Metadata is last so no updater can discover an incomplete payload set.
...
shasum -a 256 dist_electron/FORGE-2.1.0-beta.2-*.dmg
shasum -a 256 dist_electron/FORGE-2.1.0-beta.2-*.zip
shasum -a 256 dist_electron/FORGE-2.1.0-beta.2-*.blockmap
...
- [ ] GitHub exposes `FORGE beta 2.1` at `v2.1.0-beta.2`; obsolete local output and caches are cleaned.
