...
Install the verified package with `npm run install:mac`. The installer copies the universal app to a sibling staging path, verifies its executable hash, `app.asar` hash, architectures, bundle version, embedded UI commit/build date, and runtime metadata against `build-manifest.json`, then replaces `/Applications/FORGE.app` with rollback to the Trash copy if activation fails. It atomically refreshes the architecture-independent launcher at `/usr/local/bin/forge-session`. The launcher always targets the canonical application location, so it remains valid when a later packaged app replaces the bundle. `forge-session --runtime-info` reports the installed version and source commit without launching the UI.
