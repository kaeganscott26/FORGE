...
The login shell receives an explicit non-secret interactive environment: current user home/name, selected shell, terminal identity, and deduplicated Homebrew plus user-local executable paths. Provider tokens, credentials, and unrelated parent variables are not forwarded. This lets normal installed CLIs resolve in Finder-launched packaged builds without expanding the agent shell allowlist.
...
An independently installed CLI remains subject to macOS Gatekeeper. If `command -v codex` succeeds but launch shows an Apple verification dialog, FORGE has reached the executable and macOS has blocked its quarantine state. Verify the exact binary's source/signature and use the scoped macOS **Open Anyway** flow if trusted; FORGE never removes quarantine metadata or disables Gatekeeper automatically.
