...
| --- | --- | --- | --- |
| Development | `2.2.0-beta.2-dev` | none | Source and renderer development |
| Beta | `2.2.0-beta.2` | `beta`, `rc`, or normal SemVer | Public evaluation before stable |
...
The current beta target is `2.2.0-beta.2`. Stable ignores every prerelease.

## 🚀 Publication

The current beta target is **FORGE beta 2.2** with annotated tag `v2.2.0-beta.2`. An annotated prerelease tag produces a GitHub Pre-release and `beta-mac.yml`; a normal tag produces a stable release and `latest-mac.yml`. Metadata is uploaded only after the DMG, ZIP, and both blockmaps have been uploaded and hash-verified.

The beta asset family is:

- `FORGE-2.2.0-beta.2-universal.dmg`;
- `FORGE-2.2.0-beta.2-universal.dmg.blockmap`;
- `FORGE-2.2.0-beta.2-universal.zip`;
- `FORGE-2.2.0-beta.2-universal.zip.blockmap`;
- `beta-mac.yml`.
