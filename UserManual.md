...
For a local source build, first run `npm run package:mac:universal`, then `npm run install:mac`. Installation verifies `build-manifest.json`, the packaged UI provenance, and the macOS runtime metadata; it requires duplicates to be resolved, backs up the old system bundle to Trash, installs exactly `/Applications/FORGE.app`, and atomically refreshes `/usr/local/bin/forge-session`. Use `forge-session --runtime-info` to confirm the installed UI/runtime version and source commit, or run `forge-session --workspace "$PWD"` to launch the canonical application for a specific workspace.
