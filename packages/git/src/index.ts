...
export class GitService {
  ...
  async init(rootPath: string): Promise<boolean> { const git = simpleGit(rootPath); if (!await git.checkIsRepo()) { this.git = null; return false; } this.git = git; return true; }
  ...
  async pull(): Promise<void> { const git = this.ready(); const branch = (await git.branch()).current; await git.pull('origin', branch); }
  async push(): Promise<void> { const git = this.ready(); const branch = (await git.branch()).current; await git.push('origin', branch, ['--set-upstream']); }
