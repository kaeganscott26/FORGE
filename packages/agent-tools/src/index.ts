...
export function createToolRegistry(): ToolRegistry {
  ...
  registry.register(definition({ ...base, name: 'file.list', purpose: 'Discover workspace files from the root first; use a nested path only after it has been observed.', inputSchema: z.object({ path: z.string().max(4_096).default('.'), recursive: z.boolean().default(false), maxDepth: z.number().int().min(0).max(20).default(2), ...taskContext }), riskTier: 0, approval: 'automatic', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'filesystem', recordsAffectedPaths: false, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.path ?? '.', describeEffect: () => 'Read a bounded workspace directory listing, beginning at the workspace root by default.' }));
  registry.register(definition({ ...base, name: 'file.read', purpose: 'Read a supported workspace text file.', inputSchema: z.object({ path: relativePath, ...taskContext }), riskTier: 0, approval: 'automatic', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'filesystem', recordsAffectedPaths: false, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.path, describeEffect: () => 'Read text without changing the workspace.' }));
  registry.register(definition({ ...base, name: 'file.search', purpose: 'Search supported workspace text files.', inputSchema: z.object({ query: z.string().min(1).max(500), path: z.string().max(4_096).default('.'), caseSensitive: z.boolean().default(false), maxResults: z.number().int().min(1).max(MAX_SEARCH_RESULTS).default(50), ...taskContext }), riskTier: 0, approval: 'automatic', workspaceBoundary: 'required', timeoutMs: 20_000, audit: { category: 'filesystem', recordsAffectedPaths: false, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.path ?? '.', describeEffect: (input) => `Search workspace text for ${JSON.stringify(input.query)}.` }));
  registry.register(definition({ ...base, name: 'file.create', purpose: 'Create a workspace file.', inputSchema: z.object({ path: relativePath, content: z.string().max(MAX_TEXT_BYTES), reason, ...taskContext }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'filesystem', recordsAffectedPaths: true, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.path, describeEffect: () => 'Create a new file atomically.', sessionScope: (input) => input.path }));
  registry.register(definition({ ...base, name: 'file.write', purpose: 'Replace a workspace text file after showing a diff.', inputSchema: z.object({ path: relativePath, content: z.string().max(MAX_TEXT_BYTES), reason, ...taskContext }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'filesystem', recordsAffectedPaths: true, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.path, describeEffect: () => 'Atomically write the approved diff with a rollback backup.', sessionScope: (input) => input.path }));
  registry.register(definition({ ...base, name: 'file.patch', purpose: 'Apply a targeted workspace text replacement.', inputSchema: z.object({ path: relativePath, expected: z.string().min(1).max(MAX_TEXT_BYTES), replacement: z.string().max(MAX_TEXT_BYTES), replaceAll: z.boolean().default(false), reason, ...taskContext }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'filesystem', recordsAffectedPaths: true, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.path, describeEffect: () => 'Apply the displayed targeted patch atomically.', sessionScope: (input) => input.path }));
  for (const name of ['file.rename', 'file.move'] as const) registry.register(definition({ ...base, name, purpose: 'Move a workspace path without overwriting.', inputSchema: z.object({ from: relativePath, to: relativePath, reason, ...taskContext }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'filesystem', recordsAffectedPaths: true, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => `${input.from} → ${input.to}`, describeEffect: () => 'Move the path without overwriting the destination.', sessionScope: (input) => `${input.from}\0${input.to}` }));
  registry.register(definition({ ...base, name: 'directory.create', purpose: 'Create a workspace directory.', inputSchema: z.object({ path: relativePath, reason, ...taskContext }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'filesystem', recordsAffectedPaths: true, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.path, describeEffect: () => 'Create a directory inside the workspace.', sessionScope: (input) => input.path }));
  ...
  registry.register(definition({ ...base, name: 'task.inspect', purpose: 'Inspect one workspace-owned persistent task and its verified checkpoints.', inputSchema: z.object({ taskId: z.string().uuid() }), riskTier: 0, approval: 'automatic', workspaceBoundary: 'required', timeoutMs: 5_000, audit: { category: 'memory', recordsAffectedPaths: false, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.taskId, describeEffect: () => 'Read persistent task state without changing it.' }));
  registry.register(definition({ ...base, name: 'task.create', purpose: 'Create a draft workspace-owned task without executing any step.', inputSchema: taskDraft.extend({ reason }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'memory', recordsAffectedPaths: false, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.title, describeEffect: () => 'Persist a draft task and its structured steps; no executable work will start.', sessionScope: (input) => input.title }));
  for (const name of ['task.resume', 'task.pause', 'task.cancel'] as const) registry.register(definition({ ...base, name, purpose: `${name.slice(5)} a workspace-owned task after explicit approval.`, inputSchema: z.object({ taskId: z.string().uuid(), reason, trackingOnly: z.boolean().default(true) }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 20_000, audit: { category: 'memory', recordsAffectedPaths: false, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.taskId, describeEffect: () => `${name.slice(5)} task tracking without granting execution approval.`, sessionScope: (input) => input.taskId }));
  registry.register(definition({ ...base, name: 'task.checkpoint', purpose: 'Record a task checkpoint; verified checkpoints require an audit reference.', inputSchema: z.object({ taskId: z.string().uuid(), stepId: z.string().max(200).optional(), name: z.string().min(1).max(300), summary: z.string().min(1).max(4_000), verified: z.boolean().default(false), evidence: z.unknown().optional(), auditReference: z.string().max(200).optional(), reason }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'memory', recordsAffectedPaths: false, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => input.taskId, describeEffect: () => 'Persist a structured checkpoint without executing another tool.', sessionScope: (input) => input.taskId }));
  registry.register(definition({ ...base, name: 'task.handoff', purpose: 'Generate a Markdown projection of authoritative SQLite task state.', inputSchema: z.object({ taskId: z.string().uuid(), reason }), riskTier: 1, approval: 'explicit', workspaceBoundary: 'required', timeoutMs: 10_000, audit: { category: 'memory', recordsAffectedPaths: true, recordsExitCode: false, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => `.forge/handoffs for ${input.taskId}`, describeEffect: () => 'Atomically write or update a human-readable task handoff.', sessionScope: (input) => input.taskId }));
  registry.register(definition({ ...base, name: 'task.process.start', purpose: 'Start one approved task step as a detached workspace-owned process with file-backed output.', inputSchema: z.object({ taskId: z.string().uuid(), stepId: z.string().min(1).max(200), command: z.string().min(1).max(4_096), args: z.array(z.string().max(32_000)).max(500).default([]), workingDirectory: z.string().max(4_096).default('.'), timeoutMs: z.number().int().min(100).max(86_400_000).default(600_000), environment: z.record(z.string()).optional(), environmentAllowlist: z.array(z.string()).max(100).default([]), reason, expectedOutcome: z.string().min(1).max(2_000) }), riskTier: 2, approval: 'always', workspaceBoundary: 'required', timeoutMs: 30_000, audit: { category: 'shell', recordsAffectedPaths: true, recordsExitCode: true, externalDataTransfer: false }, networkAccess: false, describeTarget: (input) => `${input.taskId}/${input.stepId}: ${[input.command, ...(input.args ?? [])].map(quoteArgument).join(' ')}`, describeEffect: (input) => `${input.expectedOutcome} Output will be stored under .forge/task-output and execution may outlive the current conversation.` }));
  return registry;
}
