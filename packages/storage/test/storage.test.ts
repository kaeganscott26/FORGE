...
describe('workspace-owned conversation storage', () => {
  ...
  it('migrates an alpha.3 schema-v3 task without losing the legacy row', async () => {
    ...
    const bytes = await (await import('node:fs/promises')).readFile(join(directory, '.forge', 'metadata.sqlite')); const verify = new SQL.Database(bytes); expect(verify.exec('PRAGMA user_version')[0].values[0][0]).toBe(6); expect(verify.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='task_events'")[0].values).toHaveLength(1); verify.close();
  });
